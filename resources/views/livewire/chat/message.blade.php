        ...
        <div
                    ...
                    @if ($attachment['category'] === 'image')
                        <div class="group relative" wire:key="message-attachment-{{ $attachment['id'] }}">
                            <img src="{{ Storage::url($attachment['path']) }}" class="chat-image-bubble rounded-lg"
                                data-message-id="{{ $message->id }}" x-on:click="$dispatch('image.open')" />
                        </div>
                    @else
            ...
            <div class="flex select-none items-end justify-end gap-1" dir="ltr"
                ...
                @if ($notedAt->count() > 0)
                    <!-- Noted Batch -->
                    <div
                        class="absolute bottom-[-0.5rem] right-[-0.75rem] flex w-fit rounded-full text-green-400 dark:text-green-500">
                        <div class="relative">
                            ...
                            @if ($notedAt->count() > 1)
                                <span
                                    class="absolute -bottom-2 -right-1 z-10 rounded-full bg-green-500 px-1 text-xs text-gray-100 dark:bg-green-600 dark:text-gray-200">
                                    {{ $notedAt->count() }}
                                </span>
                            @endif
                        </div>
